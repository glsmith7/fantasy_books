...
test_2_answer = ['Continent', 'Empire', 'Kingdom', 'Principality', 'Duchy', 'County', 'March', 'Barony', 'Restrictions', 'Race', 'Default Equipment']
...
def test_sql_wrapper_all_together_now():
    ...
    test_4_answer = ['DiceFormula', 'DescriptionString', 'TestInteger01']
    ...
    assert row_names == test_5_answer
    assert final_dictionary_of_table == test_6_answer

    # check the two mini routines that just give one of the three values. Same routine, so should always be the same as above.
    assert tested_module.get_column_names(table_name="TestTable01", query = None, connection=test_connection, path = s.PATH_DEFAULT) == test_4_answer
    assert tested_module.get_row_names(table_name="TestTable01", query = None, connection=test_connection, path = s.PATH_DEFAULT) == test_5_answer